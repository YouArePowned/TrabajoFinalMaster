...
async def test_ask_user_keeps_buttons_for_websocket(tmp_path):
    ...
    response = await loop._process_message(
        InboundMessage(channel="websocket", sender_id="user", chat_id="123", content="run ls")
    )

    assert response is not None
    assert response.content == "Execute command?"
    assert response.buttons == [["Aceptar", "Rechazar"]]
